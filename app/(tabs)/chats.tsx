...
export default function ChatsScreen() {
  ...
  // --- FAB (expanded state used by coach/admin only) ---
  ...
  // --- Channel creation modal (coach/admin only) ---
  ...
  const fetchChannels = useCallback(async () => {
    ...
    // Non-admin (coach, parent, player): only channels user is a member of
  ...
  const startDM = useCallback(
    async (user: User) => {
      ...
      const selfRole = isAdmin ? 'admin' : isCoach ? 'coach' : (profile.account_type || 'parent');
      await supabase.from('channel_members').insert([
        { channel_id: newChannel.id, user_id: profile.id, display_name: profile.full_name, member_role: selfRole, can_post: true, can_moderate: isAdmin || isCoach },
    ...
    [profile, workingSeason, router, fetchChannels, isAdmin, isCoach],
  );

  // =========================================================================
  // Channel creation (coach/admin only)
  // =========================================================================
  ...
  const createChannel = useCallback(async () => {
    ...
    if (error || !newChannel) {
      setCreating(false);
      if (__DEV__) console.error('Error creating channel:', error);
      return;
    }
    ...
    <View style={s.emptyState}>
      ...
      <Text style={s.emptySubtitle}>
        {canManageChannels
          ...
          : 'Your team chats will appear here.'}
      </Text>
    </View>
  );
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      {canManageChannels ? (
        ...
        <TouchableOpacity
          style={s.fab}
          onPress={() => { setShowDmModal(true); fetchOrgMembers(); }}
          activeOpacity={0.8}
        >
          <Ionicons name="create-outline" size={26} color={BRAND.white} />
        </TouchableOpacity>
      )}
