# CC-BONUS-NODES-CONTENT
# Lynx Journey Path — Bonus Nodes for All 8 Chapters
# Status: READY FOR CC EXECUTION
# Depends on: All content phases complete, journey redesign complete
...
2. **Do NOT modify any existing rows** in skill_content, skill_quizzes, or journey_nodes. Only INSERT new rows.
3. **Do NOT modify any code files, hooks, components, or screens.**
4. **Commit after each phase.** Commit message format: `[bonus-content] Phase X: chapter Y bonus nodes`
...
Seeds 1-3 bonus nodes per chapter (16 total across 8 chapters). Bonus nodes are optional side content that branches off the main path: extra drills, conditioning exercises, film study, visualization techniques, and fun challenges. They reward XP but aren't required for chapter progression.

Each bonus node has `is_bonus = true` and connects to a new skill_content row with a drill or exercise. Some have quizzes, some don't. They use mascot images that aren't heavily used on the main path (RUSSIANTWIST, PUSHUP, GETACTIVE, WATCHINGFILM, VISUALIZE, BACKYARDPRACTICE, FOOLINGAROUNDWITHDAD, etc.)
...
Create:
```
supabase/migrations/20260316_engagement_bonus_nodes.sql
```
...
**Commit:** `[bonus-content] Phase 1: all 16 bonus nodes across 8 chapters`

---

## PHASE 2: Update MASCOT-ASSET-MAP.md

**File to modify:** `MASCOT-ASSET-MAP.md`

Add entries for all bonus node content mappings under the Skill Content table.

**Commit:** `[bonus-content] Phase 2: update asset map with bonus nodes`
...
```
## VERIFICATION REPORT: Bonus Nodes

### Content Counts (NEW rows only):
- skill_content: [count] new rows (expected 16)
- journey_nodes: [count] new rows (expected 16)

### Total Counts (all content):
- skill_content: [total] rows
- journey_nodes: [total] rows

### Per-Chapter Bonus Nodes:
- Ch 1: [count] bonus nodes
- Ch 2: [count] bonus nodes
- Ch 3: [count] bonus nodes
- Ch 4: [count] bonus nodes
- Ch 5: [count] bonus nodes
- Ch 6: [count] bonus nodes
- Ch 7: [count] bonus nodes
- Ch 8: [count] bonus nodes

### Subquery Resolution: ALL RESOLVED / [list failures]
### Image Paths Verified: [count], Mismatches: NONE / [list]
...
## IMPORTANT: You also need to paste this SQL into Supabase

Like the previous migrations, this SQL needs to be applied to the live Supabase database. After CC commits the file, Carlos will need to paste the contents of `supabase/migrations/20260316_engagement_bonus_nodes.sql` into the Supabase SQL Editor and run it.
