...
export default function LiveMatchPage() {
  ...
  return (
    <View style={s.root}>
      ...
      {/* Subs panel (tablet: right column, phone: horizontal strip) */}
      {isTablet ? (
        <View style={s.tabletSubsPanel}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {subs.map(p => (
              <TouchableOpacity
                ...
                style={s.benchItem}
                onPress={() => {
                  // Quick sub: tap bench player, then tap court player
                  setShowSubModal(true);
                  // pre-select this bench player (simplified: open modal)
                }}
              >
